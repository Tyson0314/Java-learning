...
![](http://img.topjavaer.cn/img/tcp5layer2.png)
...
![](http://img.topjavaer.cn/img/http-status-code.png)
...
![](http://img.topjavaer.cn/img/image-20211004111441594.png)
...
1. **协商加密算法** 。在`Client Hello`里面客户端会告知服务端自己当前的一些信息，包括客户端要使用的TLS版本，支持的加密算法，要访问的域名，给服务端生成的一个随机数（Nonce）等。需要提前告知服务器想要访问的域名以便服务器发送相应的域名的证书过来。
   ...
   ![](http://img.topjavaer.cn/img/image-20210921104210833.png)

2. 服务端响应`Server Hello`，告诉客户端服务端**选中的加密算法**。

   ![](http://img.topjavaer.cn/img/image-20210921105450791.png)

3. 接着服务端给客户端发来了2个证书。第二个证书是第一个证书的签发机构（CA）的证书。

   ![](http://img.topjavaer.cn/img/image-20211004172007102.png)

4. 客户端使用证书的认证机构CA公开发布的RSA公钥**对该证书进行验证**，下图表明证书认证成功。

   ![](http://img.topjavaer.cn/img/image-20210921105929268.png)

5. 验证通过之后，浏览器和服务器通过**密钥交换算法**产生共享的**对称密钥**。

   ![](http://img.topjavaer.cn/img/image-20210921110025197.png)

   ![](http://img.topjavaer.cn/img/image-20210921110155075.png)

6. 开始传输数据，使用同一个对称密钥来加解密。

   ![](http://img.topjavaer.cn/img/image-20210921110315068.png)
...
![](http://img.topjavaer.cn/img/cookie.png)
...
![](http://img.topjavaer.cn/img/session.png)
