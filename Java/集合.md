...
解决哈希冲突方法：
1、开放定址法，当发生冲突时，在冲突位置通过某种规律查找一个空单元。如线性探测再散列，顺序查看表的下一单元；二次探测再散列，在冲突位置左右进行进行跳跃式探测
2、再哈希法，同时构造多个不同的哈希函数，当发生冲突时，使用不同的哈希函数再次计算，直到冲突不再产生
3、链地址法
...
为什么使用红黑树而不使用AVL树？ConcurrentHashMap 在put的时候会加锁，使用红黑树插入速度更快，可以减少等待锁释放的时间。红黑树是对AVL树的优化，只要求部分平衡，用非严格的平衡来换取增删节点时候旋转次数的降低，提高了插入和删除的性能。
...
2. 使用hash算法计算key的索引
3. 判断索引处有没有存在元素，没有就直接CAS插入
4. 如果索引处存在元素，就在首节点加锁来保证线程安全，这里有两种情况，一种是链表形式就直接遍历到尾端插入，一种是红黑树就按照红黑树结构插入
5. 链表的数量大于阈值8，就要转换成红黑树的结构
6. 添加成功后会检查是否需要扩容
