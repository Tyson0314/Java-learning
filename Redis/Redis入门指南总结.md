...
追加值 `APPEND name " dai"`
获取字符串长度 `STRLEN name`
同时设置多个值 `MSET name tyson gender male`
同时获取多个值 `MGET name gender`
获取0索引处二进制位的值 `GETBIT name 0`
...
`HGETALL car`
当字段不存在时赋值 `HSETNX car price 400`
*HSETNX是原子操作，不存在竞态*
-  增加数字
...
- 获取列表片段`LRANGE`
`LRANGE numbers 0 2`
支持负索引 `LRANGE numbers -2 -1` *-1是最右边第一个元素*
`LRANGE numbers 0 -1`

- 删除列表指定的值 `LREM key count value`
count < 0, 则从右边开始删除前-count个值为value的元素
count > 0, 则从左边开始删除前count个值为value的元素
count = 0, 则删除所有值为value的元素 `LREM numbers 0 2`

- `LINSERT key BEFORE/AFTER pivot value` 首先从左到右寻找值为pivot的值，向列表插入value
...
`SRANDMEMBER key count` 随机获取集合里的一个元素，count大于0，则从集合随机获取count个不重复的元素，count小于0，则随机获取的count个元素有些可能相同。
`SPOP letters`
...
`ZREMRANGEBYSCORE scoreboard (80 100` 按照分数范围删除元素
...
*STORE结合EXPIRE可以缓存排序结果*
`EXPIRE resultCache 10`
...
`PUNSUBSCRIBE channel?*` 退订通过PSUBSCRIBE命令按照某种规则订阅的频道。其中订阅规则要
...
- 执行FLUSHALL命令，当没有定义自动快照条件时，执行FLUSHALL命令则不会进行快照；
- 执行复制（replication）时。当设置了主从复制后，Redis会在复制初始化时进行自动快照。
