...
select的数据列只用从索引中就能够取得，不需要**回表**进行二次查询，换句话说查询列要被所使用的索引覆盖。对于innodb表的二级索引，如果索引能覆盖到查询的列，那么就可以避免对主键索引的二次查询。
...
5. `MyISAM`不支持聚集索引，`InnoDB`支持聚集索引。
   ...
   ![](https://gitee.com/tysondai/img/raw/master/myisam-innodb-index.png)
...
[详细内容可以参考我的另一篇文章](https://www.nowcoder.com/discuss/698117?source_id=profile_create_nctrack&channel=-1)
...
![](https://gitee.com/tysondai/img/raw/master/垂直划分.jpg)
...
![](https://gitee.com/tysondai/img/raw/master/水平划分.jpg)
